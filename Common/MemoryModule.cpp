...
BuildImportTable(PMEMORYMODULE module)
{
    ...
    typedef BOOL (WINAPI *TIsBadReadPtr)(VOID*,UINT_PTR);
    TIsBadReadPtr MyIsBadReadPtr = (TIsBadReadPtr)GetProcAddress(kernel32, "IsBadReadPtr");

    if (directory->Size > 0) {
        PIMAGE_IMPORT_DESCRIPTOR importDesc = (PIMAGE_IMPORT_DESCRIPTOR) (codeBase + directory->VirtualAddress);
        for (; !MyIsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++) {
...
static HMEMORYMODULE MemoryLoadLibraryInternal(const void *data)
{
...
{
    // 标准的两参数版本，size参数用于验证
    return MemoryLoadLibraryInternal(data);
}
...
FARPROC MemoryGetProcAddress(HMEMORYMODULE module, const char *name)
{
    ...
    for (i=0; i<exports->NumberOfNames; i++, nameRef++, ordinal++) {
        if (my_stricmp(name, (const char *) (codeBase + (*nameRef))) == 0) {
            idx = *ordinal;
            break;
        }
    }
