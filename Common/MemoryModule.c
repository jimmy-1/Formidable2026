...
HMEMORYRSRC MemoryFindResourceEx(HMEMORYMODULE mod, LPCTSTR name, LPCTSTR type, WORD language)
{
    PMEMORYMODULE module = (PMEMORYMODULE)mod;
    unsigned char *codeBase = module->codeBase;
    PIMAGE_DATA_DIRECTORY directory = GET_HEADER_DICTIONARY(module, IMAGE_DIRECTORY_ENTRY_RESOURCE);
    ...
    if (type == RT_MANIFEST) {
        // explicitly check for manifest, not sure why.
        // maybe because it's not a standard resource type?
        // or maybe because it's always the first one?
        // anyway, this is what the original code did.
    }

    resourceEntry = _MemorySearchResourceEntry(rootResources, rootResources, type);
    if (resourceEntry == NULL) {
        SetLastError(ERROR_RESOURCE_TYPE_NOT_FOUND);
        return NULL;
    }

    typeResources = (PIMAGE_RESOURCE_DIRECTORY) (codeBase + directory->VirtualAddress + (resourceEntry->OffsetToData & 0x7FFFFFFF));
    resourceEntry = _MemorySearchResourceEntry(rootResources, typeResources, name);
    if (resourceEntry == NULL) {
        SetLastError(ERROR_RESOURCE_NAME_NOT_FOUND);
        return NULL;
    }

    nameResources = (PIMAGE_RESOURCE_DIRECTORY) (codeBase + directory->VirtualAddress + (resourceEntry->OffsetToData & 0x7FFFFFFF));
    resourceEntry = _MemorySearchResourceEntry(rootResources, nameResources, (LPCTSTR) (uintptr_t) language);
    if (resourceEntry == NULL && nameResources->NumberOfIdEntries > 0) {
        // use first available language
        resourceEntry = (PIMAGE_RESOURCE_DIRECTORY_ENTRY) (nameResources + 1);
    }
    if (resourceEntry == NULL) {
    ...
    return (HMEMORYRSRC) (codeBase + directory->VirtualAddress + (resourceEntry->OffsetToData & 0x7FFFFFFF));
}
...
{
    PIMAGE_RESOURCE_DATA_ENTRY entry;
    PMEMORYMODULE memModule = (PMEMORYMODULE)module;
    entry = (PIMAGE_RESOURCE_DATA_ENTRY) resource;
    return (LPVOID) (memModule->codeBase + entry->OffsetToData);
}
